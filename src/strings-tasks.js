...
function isPalindrome(str) {
  const repl = /[\W]/g;
  const lowStr = str.toLowerCase().replace(repl, '');
...
function findLongestWord(sentence) {
  ...
  const longestWord = strArr.sort((a, b) => {
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    reversedString.push(reversedWords);
  }
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => cipher[alphabetStr.indexOf(letter)]
  );
}
